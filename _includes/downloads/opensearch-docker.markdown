...
2. Download [docker-compose.yml](/samples/docker-compose.yml) into your desired directory
  ...
  For a successful security demo configuration setup, set the initial admin password using the `OPENSEARCH_INITIAL_ADMIN_PASSWORD` environment variable. Create a `.env` file in the same folder as your `docker-compose.yml` and include the variable with a strong password value. This configuration will be automatically utilized during the setup process.
