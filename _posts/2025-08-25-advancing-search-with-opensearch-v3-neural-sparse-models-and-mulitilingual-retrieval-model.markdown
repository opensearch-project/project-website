...
* **A new multilingual retrieval model** – introducing the first multilingual neural sparse retrieval model in OpenSearch.
<br><br>
...
The v2 series made neural sparse search widely accessible by significantly reducing model parameters and improving ingestion throughput, without compromising too much on search relevance. However, as search workloads and datasets have grown in complexity, certain challenges emerged:

* **Relevance bottleneck**: While v2 delivered strong efficiency and solid performance, its inference-free design still trailed behind well-trained siamese dense or sparse retrievers in retrieval quality.
* **Limited teacher guidance**: v2 relied primarily on heterogeneous bi-encoder teachers for distillation, without leveraging the richer ranking signals from stronger models such as large language models (LLMs).

These limitations motivated us to rethink both training strategies and model architectures for the next generation.
...
The GTE backbone strengthens semantic representation and handles much longer inputs, while LLM-based teacher signals offer richer ranking guidance. This combination allows v3-gte to deliver the highest relevance scores among all OpenSearch sparse retrievers.


With these advancements, the v3 series delivers substantial improvements in search relevance while preserving the hallmark speed, efficiency, and inference-free advantages of previous generations. This ensures that OpenSearch users can achieve state-of-the-art retrieval performance without compromising scalability or latency.
