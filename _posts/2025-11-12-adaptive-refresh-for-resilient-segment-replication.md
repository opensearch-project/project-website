...
meta_description: This blog dives into the searcher refresh mechanism in Lucene, how adaptive refresh works, and the strategies to implementing it successfully in a highly replicated search engine.
excerpt: Segment replication is a powerful index replication strategy for high QPS workloads that allows physical isolation and decoupling of indexing and search workloads, rapid replica failovers, and seamless point in time restores. However, this efficiency comes with its own unique challenges. During high indexing bursts, or if replication gets delayed due to network issues, the system can accummulate large replication checkpoints. Searchers (replica shards) struggle to absorb these checkpoints experiencing high page faults, thrashing for in-flight queries, and degraded search performance. This blog describes how to address these issues and build a resilient segment replicated system using Lucene's powerful new hooks for "Adaptive Refresh". 
---
...
Highly replicated systems are the foundation of modern distributed computing. In search engines, this typically means serving the same index on a large number of 'searcher' instances or replicas. By distributing query traffic across these replicas, we can scale the service beyond the throughput of a single instance. Further, provisioning replicas on geographically separated physical instances helps with high availability – one bad instance does not take the service down, and the service can survive outage in a data center.
...
In well architected systems, indexer and replicas are decoupled via remote storage. Indexers publish periodic checkpoints to a remote store (like s3). Replicas poll the store and fetch the latest available checkpoint. Remote stores provide durability; if a replica crashes or restarts, it can fetch and load the latest checkpoint. It also decouples indexers and replicas: indexers can continue to publish checkpoints at a fixed pace without getting held up by lags or issues on replicas. 
...
Lucene provides concrete implementations like `SearcherManager` and `SearcherTaxonomyManager` that hold a single active `IndexSearcher` (and optionally a `TaxonomyReader` for faceted search). These managers swap the searcher atomically during refresh while using reference counting to ensure active queries on the old searcher finish safely before resources are closed. As a user, you typically call `acquire()` to get a reference to the searcher, run your query, then call `release()`, to let go of the searcher reference.

Here's how a typical refresh unfolds in a segment replicated systems. Indexer machines produce new commits over time, which are copied over to replicas via checkpoints. The replica searcher is then *refreshed* on the new checkpoint using the `refreshIfNeeded` API. Internally, this API invokes a `DirectoryReader.openIfChanged()` on the directory holding commits from the downloaded checkpoint, which in turn returns an `IndexReader` on the latest commit available in the directory. 
...
Replica receives this checkpoint and attempts to refresh on commit 7. High update traffic and merge activity on indexers can cause the index in commit 7 to be significantly different from commit 1. This is not unreasonable, after all, we did just skip through five commits worth of indexing activity. While this may seem efficient in theory – one download, one refresh, and the replica is up to date, refreshing on a large chunk of new index segments can significantly stress the system. 
...
In the steps below, we describe a detailed strategy that let’s you surgically download and refresh only the bytes you will refresh on, and in increments that do not stress your replicas.
...
**3) Select optimal commit for refresh**
Using our example of a threshold based selection strategy, replica evaluates the commit diffs for each available commit, and selects the newest commit that is below the threshold. Once a commit is identified for refresh, replica downloads all the files referenced by that commit and refreshes on it.

It is worth mentioning that you’ll need to handle the edge case where none of the commits are below your statically defined threshold, which could happen due massive update storms even in the best effort-15 second window, or aggressive merges that lead to segment files greater than the defined threshold.  We have found that simply falling back to picking up the next commit is the safest path forward in such instances.
...
Check deltas vs current (1), walking from newest to oldest commit:
...
The newest commit with a delta below our threshold of 5gb is **“commit 4”**. We download files for that commit and refresh the searcher on it.


#### **Pass 2 (current searcher commit = 4, latest commit = 7):**

Since we are not on the latest commit yet, we run through the loop again. This time however, searcher is on commit=4, and so the bytes delta with subsequent commits is smaller.
...
We find that “commit 6” is now within our safe to refresh threshold of 5GB. We download relevant files and refresh on commit 6, bringing our searcher closer to the latest commit in checkpoint. Replica can also delete the now unused segment files from the old point-in-time searcher, freeing up some disk space.


#### **Pass 3 (current searcher commit = 6, latest commit = 7):**

Running through the same loop, we now find that “commit=7” is now conveniently within our safe to refresh threshold. We fetch and refresh on the commit, thus getting our searchers on the freshest data for the index.
...
Thus, instead of one big jump, the replica moves in bounded bites, each refresh capped by a size threshold, instead of a single 11 GB transfer, the replica progresses through three smaller steps of 5 GB, 5 GB, and 3 GB. Even though the total new bytes are more (13 GB in this case), each individual refresh is smaller and cheaper, and the system avoids sharp spikes in network, memory, or CPU load.
