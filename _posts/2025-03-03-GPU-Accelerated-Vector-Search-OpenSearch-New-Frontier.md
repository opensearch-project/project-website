...
2. During flush/merge operations, the Vector Engine:
  - Uploads vectors to the object store.
  - Initiates a GPU-accelerated index build request to GPU fleet.
...
These results show that GPU acceleration **improves indexing speed by 9.3x** while **reducing costs by 3.75x**. Additionally, **CPU utilization is 2.5x lower** compared to the baseline, while indexing throughput improves by **2x**. Because vector index builds no longer occur on CPUs, the OpenSearch cluster can handle higher indexing traffic without additional scaling, making it more efficient for heavy workloads.
...
GPU-accelerated vector indexing marks a major milestone in OpenSearch's evolution to support large-scale AI workloads. Our benchmarks show **9.3x faster index build times** at **3.75x lower cost** compared to CPU-based solutions, reducing billion-scale index builds from days to hours. While GPU instances cost **1.5x more** than CPU counterparts, the substantial speedup delivers a strong price-performance advantage—further amplified by the shared GPU fleet model for multi-tenant use.
