...
_Note_: Since DeepSeek R1 is open source, you can host it on your preferred machine learning platform. For example, you can deploy it on AWS (see AWS documentation at [AWS News Blog](http://aws.amazon.com/blogs/aws/deepseek-r1-models-now-available-on-aws)). To connect to your hosted model, update the `endpoint` and `credentials` parameters in your configuration. For detailed deployment instructions, please refer to our guides at the end of the blog.
