...
excerpt: Random rotation is a useful tool to make our data more standard before feeding it into an approximate nearest neighbor search algorithm. While it has a computational cost that needs to be taken into account, it can remove spurious axis correlations that lead to reduced accuracy.
has_math: true
has_science_table: true
---

Modern vector search systems, like those used in retrieval augmented generation (RAG) pipelines, need to handle a wide variety of datasets with rich and diverse structures and properties. It is often difficult to predict how a search algorithm would perform on a new dataset. One way to promote performance predictability is through transformations that standardize the data, making it more amenable to the approximate nearest neighbor (ANN) algorithm down the pipeline. 

OpenSearch 3.2 introduces random rotation as a data standardizing transformation, which is helpful particularly in the context of [binary quantization](https://docs.opensearch.org/latest/vector-search/optimizing-storage/binary-quantization/#enhancing-search-quality-with-adc-and-rr). A random rotation simply rotates all our data and query vectors to a random orientation. The following is an example of an ellipsoidal dataset in two dimensions, before and after a random rotation (randomly chosen to be 130.6 degrees). 
