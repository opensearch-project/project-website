...
In the following test running against the query "How can the orientation of texture be characterized?" on the same index `multimodal_docs`, on the left side, it's using the `colpali_search_pipeline` search pipeline and on the right side, it's using the `titan_embedding_search_pipeline` search pipeline. The expected result is row 7 page, which is ranking no.1 when using the colpali model on the left panel.
