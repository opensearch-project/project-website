...
Observe that you get data back.  You will see that the `genres` field has multiple values (array) and in fact “Fantasy” is in the result set.  There is NO field called `actors` and the `directors` field has been masked:
