...
```
GET _templates/movies_template
```
...
Clean up the transferred data from the movies index so we have separated indexes to show fine grained access controls.  Issue the following command:
...
Create a user called `read_only_index_level` user. Scroll to the bottom of the form, leave the other fields alone for now and click on the create button and observe that the user got created. Do not map them.
...
Assign the global tenant in Tenant permissions and create the role by clicking the Create button at the bottom right of the form.
...
![Search with read_only](/assets/media/tutorials/opn305/_search-read_only.png){: .img-fluid}
...
Repeat this exercise for the `movies_subset` index.  You should observe similar behaviors.  At this point in time, your user cannot do anything other than view an index from Dev Tools.  What if you wanted to view the data in the discover tab?  If you navigate to the discover tab, you will find that nothing shows up.  What permissions do you think can enable this user to view things in the discover tab?

![Discover](/assets/media/tutorials/opn305/discover.png){: .img-fluid}

As you click on Discover link, there will be nothing on the browser but an empty work area.

![Discover is empty](/assets/media/tutorials/opn305/discover-empty.png){: .img-fluid}

Log out as the read only user and log back in as admin.  Navigate to the role and add the following permission `indices:admin/resolve/index`.  Add the permission and update the role.

![Adding indices:admin/resolve/index](/assets/media/tutorials/opn305/admin-resolve-index.png){: .img-fluid}

Next, find the role called `kibana_user` and map the `read_only_index_level` to the role.

![map the read_only_index_level](/assets/media/tutorials/opn305/kibana_user.png){: .img-fluid}

Click on the Map button at the bottom right of the form and observe the following:

![map the read_only_index_level](/assets/media/tutorials/opn305/kibana-user-observe.png){: .img-fluid}
